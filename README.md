...
- `http://localhost:3000/data-sync`: 개발용 소상공인 상가정보 CSV 동기화
...
- 예시 이름: `PUBLIC_DATA_SERVICE_KEY`, `SPRING_DATASOURCE_URL`, `SPRING_DATASOURCE_USERNAME`, `SPRING_DATASOURCE_PASSWORD`, `NEXT_PUBLIC_API_BASE_URL`, `NEXT_PUBLIC_KAKAO_MAP_APP_KEY`, `LOCALBIZ_STORE_SYNC_MAX_ROWS_PER_IMPORT`
...
이번 단계의 데이터 동기화는 실시간 OpenAPI 호출이 아니라 CSV 파일 기반 import입니다. 소상공인 상가정보 CSV를 `/data-sync` 화면에서 업로드하면 Spring Boot API가 header 기반으로 파싱하고, `external_store_id + source_system` 기준으로 `stores` table에 upsert합니다.
...
- 저장소, 캐시, 배치 동기화 전략 수립
- CSV import 실패 row 다운로드와 대용량 import 최적화
- Spring Scheduler 기반 주기적 동기화 검토
