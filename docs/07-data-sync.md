...
데이터 동기화 기능은 소상공인 상가정보 CSV 파일을 Spring Boot API로 업로드하고, CSV row를 `stores` table에 반영하는 개발용 import 도구입니다. 이번 단계에서는 실시간 OpenAPI 호출, 스케줄러 자동 동기화, 인증/인가는 구현하지 않습니다.
...
CSV import 결과는 `sync_logs` table에 저장합니다.

주요 필드:

- `sync_type`: 현재는 `STORE_CSV_IMPORT`
