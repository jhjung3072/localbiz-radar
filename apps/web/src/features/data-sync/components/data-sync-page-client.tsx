...
type DataSyncPageClientProps = {
  initialData?: AdminDataSyncBffData | null;
};

export function DataSyncPageClient({ initialData }: DataSyncPageClientProps) {
  return (
    <Suspense fallback={<DataSyncGuardFallback />}>
      <AdminGuard>
        <DataSyncContent initialData={initialData} />
      </AdminGuard>
    </Suspense>
  );
}
...
function DataSyncContent({ initialData }: DataSyncPageClientProps) {
  ...
  const masterStatusQuery = useQuery({
    ...
    initialData: initialData?.masterStatus,
  });
