...
type AdminOpsPageClientProps = {
  initialData?: AdminOpsBffData | null;
};

export function AdminOpsPageClient({ initialData }: AdminOpsPageClientProps) {
  return (
    <Suspense fallback={<LoadingState title="운영 상태를 확인하는 중입니다" />}>
      <AdminGuard>
        <AdminOpsContent initialData={initialData} />
      </AdminGuard>
    </Suspense>
  );
}

function AdminOpsContent({ initialData }: AdminOpsPageClientProps) {
  useEffect(() => {
